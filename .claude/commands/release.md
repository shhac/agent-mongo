...
You are performing a release of the `agent-mongo` CLI. Follow these steps exactly.
...
```bash
rm -f release/agent-mongo-* release/checksums-sha256.txt
...
for bin in agent-mongo-darwin-arm64 agent-mongo-darwin-x64 agent-mongo-linux-x64 agent-mongo-linux-x64-musl agent-mongo-linux-arm64 agent-mongo-linux-arm64-musl; do
  tar czf "${bin}.tar.gz" "$bin"
done
# Regenerate checksums to include tarballs
shasum -a 256 *.tar.gz agent-mongo-windows-x64.exe > checksums-sha256.txt
```
...
```bash
gh release create v<NEW_VERSION> release/*.tar.gz release/agent-mongo-windows-x64.exe release/checksums-sha256.txt \
  --title "v<NEW_VERSION>" \
  --notes "<release notes>"
```
...
```bash
ls ../homebrew-tap/Formula/agent-mongo.rb
```

**If it doesn't exist:** Skip this step and tell the user: "Homebrew tap not found at `../homebrew-tap/Formula/agent-mongo.rb`. Update the formula manually or create one in the homebrew-tap repo."

**If it exists:** Read the SHA256 checksums from `release/checksums-sha256.txt` and update the formula:

1. Read `../homebrew-tap/Formula/agent-mongo.rb`
2. Update:
...
4. Commit and push:
   ```bash
   cd ../homebrew-tap
   git add Formula/agent-mongo.rb
   git commit -m "agent-mongo <NEW_VERSION>"
   git push
   ```
