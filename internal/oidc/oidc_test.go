...
func testClient(t *testing.T, idp *mockIDP, at time.Time) (*Client, *[]time.Duration) {
	t.Helper()
	var slept []time.Duration
	return &Client{
		HTTP:  idp.server.Client(),
...
func TestDiscover(t *testing.T) {
	...
	provider, err := client.Discover(context.Background(), idp.server.URL)
	if err != nil {
		t.Fatalf("Discover: %v", err)
	}
	if provider.TokenEndpoint != idp.server.URL+"/token" {
		t.Errorf("TokenEndpoint = %q", provider.TokenEndpoint)
	}
	if provider.DeviceAuthorizationEndpoint != idp.server.URL+"/device" {
...
func TestDiscoverTolatesATrailingSlash(t *testing.T) {
	...
	if _, err := client.Discover(context.Background(), idp.server.URL+"/"); err != nil {
		t.Fatalf("Discover: %v", err)
	}
}
...
func TestDiscoverRejectsAProviderWithoutTheDeviceGrant(t *testing.T) {
	...
	_, err := client.Discover(context.Background(), idp.server.URL)
	if err == nil {
...
func TestStartDeviceAuth(t *testing.T) {
	...
	if got := idp.lastForm.Get("scope"); got != "openid offline_access" {
		t.Errorf("scope = %q, want the scopes space-joined", got)
	}
	if got := idp.lastForm.Get("client_id"); got != "client-1" {
...
func startAuth(t *testing.T, client *Client, idp *mockIDP) (Provider, DeviceAuth) {
	t.Helper()
	provider, err := client.Discover(context.Background(), idp.server.URL)
	if err != nil {
		t.Fatalf("Discover: %v", err)
...
func pollToken(t *testing.T, client *Client, idp *mockIDP) Token {
	t.Helper()
...
func pollTokenErr(t *testing.T, client *Client, idp *mockIDP) (Token, error) {
	t.Helper()
	provider, auth := startAuth(t, client, idp)
